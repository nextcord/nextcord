...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = nextcord.Client(activity=nextcord.Game(name='my game'))

    # or, for watching:
    activity = nextcord.Activity(name='my activity', type=nextcord.ActivityType.watching)
...
If you want to upload an image it's as simple as: ::
    ...
    await channel.send(file=nextcord.File('my_file.png'))

If you have a file-like object you can do as follows: ::

    with open('my_file.png', 'rb') as fp:
        await channel.send(file=nextcord.File(fp, 'new_filename.png'))

To upload multiple files, you can use the ``files`` keyword argument instead of ``file``\: ::

    my_files = [
        nextcord.File('result.zip'),
        nextcord.File('teaser_graph.png'),
    ]
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=nextcord.File(data, 'cool_image.png'))
...
Quick example: ::
    ...
    # no ID, do a lookup
    emoji = nextcord.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
Quick example: ::
    ...
    guild = nextcord.utils.get(client.guilds, name='My Server')
...
Example: ::
    ...
    async def main():
        ...
        await bot.start(token='your_token')
