...
class SizedDict(dict):
    ...
    def __init__(self, maxsize: Optional[Union[float, int]] = None):
        if not maxsize:
            maxsize = float("inf")
        else:
...
class BetterTTLCache(dict):
    ...
    def __init__(self, maxsize: Optional[int] = None, ttl: Optional[Union[int, float]] = None):
    ...
    def set_with_persist_ttl(self, key, val):
        self.__expire__()
        if len(self) >= self.maxsize:
            for item in self:
                del self[item]; break
